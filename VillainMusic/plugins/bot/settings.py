...
from VillainMusic.utils.decorators.admins import ActualAdminCB
from VillainMusic.utils.decorators.language import language, languageCB
from VillainMusic.utils.inline.settings import (
    auth_users_markup,
    playmode_users_markup,
    setting_markup,
    vote_mode_markup,
)
from VillainMusic.utils.inline.start import private_panel
...
async def settings_back_markup(client, CallbackQuery: CallbackQuery, _):
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        ...
        try:
            if CallbackQuery.message.photo or CallbackQuery.message.video:
                return await CallbackQuery.edit_message_caption(
                    _["start_2"].format(CallbackQuery.from_user.mention, app.mention, UP, DISK, CPU, RAM, CallbackQuery.from_user.username or "None"),
                    reply_markup=InlineKeyboardMarkup(buttons),
                )
            else:
                return await CallbackQuery.edit_message_text(
